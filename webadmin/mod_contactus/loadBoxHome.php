...
$sql = $sql."  WHERE ".$mod_tb_root."_masterkey ='".$_REQUEST['masterkey']."'  ORDER BY ".$mod_tb_root."_id DESC LIMIT 0,5 ";
  ...
  <tr>
    ...
    <td  align="left"  valign="top"><a href="../<?=$mod_fd_root?>/viewContant.php?masterkey=<?=$_REQUEST['masterkey']?>&menukeyid=<?=$_REQUEST['menukeyid']?>&viewID=1&inputLt=<?=$_SESSION[$valSiteManage.'core_session_language']?>&valEditID=<?=$valID?>" class="moreDetailAb"><?=txtLimit($valNameShow,45)?></a></td>
  </tr>
  <?  } ?>
  </table>
