...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadHTh';
	if(!empty($_FILES['fileToUploadHTh']['error'])){
		switch($_FILES['fileToUploadHTh']['error']){
	...
	}elseif($_FILES['fileToUploadHTh']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$inputGallery=$_FILES['fileToUploadHTh']['tmp_name'];
			$arrfile=$_FILES['fileToUploadHTh'];
			$ERROR=$_FILES['fileToUploadHTh']['error'];
			$TIME=time();
			if(!$ERROR) {
			$filename="pic-HTh-".$_REQUEST['myID'];
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUploadSite(\'deletePicInsertHTh.php\',\'boxPicHTh\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picHTh\" type=\"hidden\" id=\"picHTh\" value=\"$picname\" /> ";
