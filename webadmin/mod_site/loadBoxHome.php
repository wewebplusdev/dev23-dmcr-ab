...
$sqlChk = "SELECT " . $core_tb_menu . "_id," . $core_tb_menu . "_linkpath FROM " . $core_tb_menu;
 $sqlChk = $sqlChk . "  WHERE " . $core_tb_menu . "_masterkey ='" . $_REQUEST['masterkey'] . "' AND  " . $core_tb_menu . "_id ='" . $_REQUEST['menukeyid'] . "'  ";
...
$sql = "SELECT " . $mod_tb_set . "_id," . $mod_tb_set . "_subject," . $mod_tb_set . "_credate  FROM " . $mod_tb_set;
$sql = $sql . "  WHERE " . $mod_tb_set . "_masterkey ='" . $_REQUEST['masterkey'] . "'  ORDER BY " . $mod_tb_set . "_id DESC LIMIT 0,5";
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        while ($row = wewebFetchArrayDB($coreLanguageSQL,$query)) {
            ...
            <tr>
                ...
                <td  align="left"  valign="top"><a href="../<?= $mod_fd_root ?>/loadSet.php?masterkey=<?= $_REQUEST['masterkey'] ?>&menukeyid=<?= $_REQUEST['menukeyid'] ?>&viewID=1&inputLt=<?= $_SESSION[$valSiteManage . 'core_session_language'] ?>&valEditID=<?= $valID ?>" class="moreDetailAb"><?= txtLimit($valNameShow, 45) ?></a></td>
            </tr>
        <? } ?>
    </table>
