...
<table width="96%" border="0" cellspacing="0" cellpadding="0" align="center"   class="tbBoxListwBorder">
  ...
  <?
// SQL SELECT #########################
$sql = "SELECT ".$mod_tb_root_subgroup."_id,".$mod_tb_root_subgroup."_subject,".$mod_tb_root_subgroup."_lastdate,".$mod_tb_root_subgroup."_status,".$mod_tb_root_subgroup."_subjecten  FROM ".$mod_tb_root_subgroup;
...
if($count_record>0) {
  ...
  <tr>
   ...
   document.myFormHome.valEditID.value=<?=$valID?>;
    viewContactNew('viewSubGroup.php');" ><?=$valName?></a>
   </td>
  </tr>
</table></td>
  ...
  <tr>
   ...
   document.myFormHome.valEditID.value=<?=$valID?>;
    viewContactNew('viewSubGroup.php');" ><?=$valNameEn?></a></td>
  </tr>
</table>    </td>
<? }?>
...
<a href="javascript:void(0)"  onclick="changeStatus('load_waiting<?=$valID?>','<?=$mod_tb_root_subgroup?>','<?=$valStatus?>','<?=$valID?>','load_status<?=$valID?>','../<?=$mod_fd_root?>/statusMg.php')" ><span class="<?=$valStatusClass?>"><?=$valStatus?></span></a>
