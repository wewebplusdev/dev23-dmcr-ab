<?php
$menuActive = "register";
$menuAction = "registerDetail_new";


$listjs[] = '<script src="https://cdnjs.cloudflare.com/ajax/libs/1000hz-bootstrap-validator/0.11.5/validator.min.js"></script>';

$listjs[] = '<script type="text/javascript" src="front/controller/script/'.$menuAction.'/js/main.js'."?u=".date("YdmHis").'"></script>';
$listcss[] = '<link rel="stylesheet" type="text/css" href="front/template/default/public/css/front-qa-new.css'."?u=".date("YdmHis").'" />';
$listjs[] = '<script type="text/javascript" src="front/controller/script/'.$menuAction.'/js/ajaxfileupload.js'."?u=".date("YdmHis").'"></script>';
$listjs[] = '<script type="text/javascript" src="front/controller/script/'.$menuAction.'/js/jquery.uploadfile.js'."?u=".date("YdmHis").'"></script>';
$listjs[] = '<script type="text/javascript" src="front/controller/script/'.$menuAction.'/js/formfile.js'."?u=".date("YdmHis").'"></script>';
// $myRand = time() . rand(111, 999);
// $smarty->assign("myRand",$myRand);


if ($url->segment[0] == $menuAction && is_numeric($url->segment[1])) {
	...
	$infoFormSectionNew =callFormSectionNew($id_quse);
	$infoFormSectionNewrecordCount = $infoFormSectionNew->recordCount();
	if($infoQuestionare->fields['sectionid']==1 && $infoFormSectionNewrecordCount>=1){
		$smarty->assign("sectionidMain",1);
	}else{
		$smarty->assign("sectionidMain",0);
	}
    $smarty->assign("infoFormSectionNew",$infoFormSectionNew);
	
	//print_pre($infoQuestionare->fields['sectionid']);
    ...
    if($_POST['action']=="addnew"){
            ...
            if ($chkTotal == $infoQuestionare->fields['quality'] || $infoQuestionare->_numOfRows == 0 || $timeOut == 1) {
                header("Location:" . $linklang . "/".$menuAction."/error");
                exit();
            }
        ...
        //     $Mid = $db->Insert_ID();
            ...
            header("Location:" . $linklang . "/".$menuAction."/save");
            exit();
    ...
    if(is_numeric($url->segment[2])){
        ...
        $infoQuestionare = callFormDetailCheck($id_quse);
    ...
    if(!empty($_GET['WP'])){
       ...
       $expo = explode("&",$linkget);
        ...
        require_once _DIR . '/front/controller/script/'.$menuAction.'/form.php';
