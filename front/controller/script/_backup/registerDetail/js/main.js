...
function checkData(thisObj, vchar, valueInput){
    ...
    // console.log(valueOption.filter(x => x === '3').length);
    ...
    if (valueOption !== null) {
        ...
        if (valueOption.filter(x => x === '3').length > 0 && valueOption.filter(x => x === '4').length == 0) {
            ...
            if($.isNumeric(valueInput)){
                ...
                if (typeof valueReturn[valueID] === "undefined") {
                    valueReturn[valueID] = '';
                }else{
...
$.fn.dataTable.SelectPager = function ( context ) {
  ...
  if (tableId !== '') {
    ...
    nPaging.setAttribute('class', 'dataTables_paginate_jump');
  nInput.style.display = "inline";
  nPage.innerHTML = "Page ";
...
var table = $('#dataList')
  ...
  .DataTable({
    dom: 'lSftrip',
    "oLanguage": {
        "sUrl": "https://register.dmcr.go.th/front/controller/script/registerDetail/js/Thai.json?u=20211904085252"
    },
  });
...
function checkDisableReq() {
	
	var indexchb =0;
	 $( "input:checkbox[class=required-chb]").each(function() {
		var idchb = $( this ).attr("data-id");
		var check_chb = $('ul.item-list.'+idchb+' :checkbox:checked').length > 0;
		if(check_chb==false){
			indexchb++;
		}
	});
		if(indexchb>=1){
			$( "input:checkbox[class=required-chb]").prop('required',true);
		}else{
			$( "input:checkbox[class=required-chb]").prop('required',false);
		}
		
		 $("#myFormQuse_form").validator('validate');  
...
// #### Start Check Box V alidate ####

$( "input:checkbox[class=required-chb]").each(function() {
	$( "input:checkbox[class=required-chb]").prop('required',true);
});
 
$( "input:checkbox[class=required-chb]" ).click(function(e) {

	checkDisableReq() ;

});

// #### End Check Box V alidate ####
