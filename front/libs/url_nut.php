...
class url {
    ...
    public $listcheckurl = array("-category", "-product","-store");
    public $optionurl;
    ...
    public function __construct() {
        ...
        $this->url = str_replace(_DIR . '/', '', str_replace('//', '/', $_SERVER['DOCUMENT_ROOT'] . $_SERVER['REQUEST_URI']));

        $urlall = explode("?", $this->url);
        $segment = cleanArray(explode('/', $urlall[0]));
        $this->segment = $segment;

      //  print_pre($url_show_lang);
    
        if (!empty($url_show_lang)) {
           // print_pre($this->segment);
